...
static void updater_ui_print(UpdaterInfo *cookie, const char *fmt, ...)
{
    char buf[256];
    va_list ap;
    va_start(ap, fmt);
    ...
    fprintf(cookie->cmd_pipe, "ui_print %s\n", buf);
    fprintf(cookie->cmd_pipe, "ui_print\n");
}
...
static int extractedFileCount = 0;
static void ExtractDirCallback(const char *file, void* cookie)
{
	extractedFileCount++;
	if (0==extractedFileCount%50) { // almost 800 file now
		updater_ui_print(cookie, "Extracted %d files\n", extractedFileCount);
	}
}
...
Value* PackageExtractDirFn(const char* name, State* state,
    ...
    bool success = mzExtractRecursive(za, zip_path, dest_path,
                                      ...
                                      0, &timestamp, // Fix symlink error by frank???
                                      //NULL, NULL);
                                      ExtractDirCallback, state->cookie);
    updater_ui_print(state->cookie, "Total extracted %d files\n", extractedFileCount);
    free(zip_path);
    free(dest_path);
