...
prepend_title(char** headers) {
	char* title[] = { "TNOS system utility 1.0",
			"2010 Copyright Tsinghua University",
			"",
			NULL };
...
is_internet_connected() {
	char gateway[256];
	property_get("net.rmnet0.gw",gateway,"1");
	if(strlen(gateway) == 1) {
		return 0;
	}
	return 1;
...
enter_remote_update(){
	...
	if(pc < 0) {
		...
		switch(chosen_item) {
			case 0: 
				if(execl("/sbin/sh","sh","/system/etc/tnosupdate.sh",NULL) < 0) {
					perror("execute failed");
				}
				break;
			case 1:
				if(execl("/sbin/sh","sh","/system/etc/tnosupdate-1.sh",NULL) < 0) {
					perror("execute failed");
				}
				break;
			default:
				...
				if(!is_internet_connected() ) {
					do {
						...
						if(pc < 0) {
							...
							if(execl("/sbin/sh","sh","/system/etc/init.connect",NULL) < 0) {
								perror("execute failed");
							}
						}else {
							wait(NULL);
						}
					} while(is_connect_retry_needed());
				}
				enter_remote_update();
				break;
		}
