...
static int responseSimStatus(Parcel &p, void *response, size_t responselen) {
    ...
    if (responselen != sizeof (RIL_CardStatus)) {
        LOGE("invalid response length %d expected %d\n",
...
static void debugCallback (int fd, short flags, void *param) {
    ...
    LOGE("****libril.so****begin debugCallback2");
    if (acceptFD < 0) {
    ...
    LOGE("****libril.so****option[%d]",atoi(args[0]));
    switch (atoi(args[0])) {
        case 0:
        ...
        case 6:
            ...
            actData[0] = args[1];
            issueLocalRequest(RIL_REQUEST_SETUP_DATA_CALL, &args[1],
                              6*sizeof(actData));
            break;
        case 7:
        ...
        case 10:
            ...
            break;
        case 11:
            LOGI("Debug port: Get IMSI");
            issueLocalRequest(RIL_REQUEST_GET_IMSI, NULL, 0);
            break; 
        default:
...
RIL_register (const RIL_RadioFunctions *callbacks) {
    ...
    LOGE("****libril.so****open RIL_DEBUG socket");
    s_fdDebug = android_get_control_socket(SOCKET_NAME_RIL_DEBUG);
    if (s_fdDebug < 0) {
        LOGE("Failed to get socket '" SOCKET_NAME_RIL_DEBUG "' errno:%d", errno);
        fprintf(stderr,"Failed to get socket '" SOCKET_NAME_RIL_DEBUG "' errno:%d", errno);
        exit(-1);
    }

    ret = listen(s_fdDebug, 4);

    fprintf(stderr,"****libril.so****listen RIL_DEBUG socket");
    if (ret < 0) {
        LOGE("Failed to listen on ril debug socket '%d': %s",
             s_fdDebug, strerror(errno));
        fprintf(stderr,"(Failed to listen on ril debug socket '%d': %s",
             s_fdDebug, strerror(errno));
        exit(-1);
    }
...
RIL_onRequestComplete(RIL_Token t, RIL_Errno e, void *response, size_t responselen) {
    ...
    if ( pRI->pCI->requestNumber == RIL_REQUEST_GET_IMSI ){
        LOGD ("IMSI:[%s]",response);
        property_set(PROPERTY_RIL_IMSI, (char *)response);
    }
    if (pRI->local > 0) {
